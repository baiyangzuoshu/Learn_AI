...
export class DoubleRotateCollapse implements CollapseStrategy {
    collapse(delayTime: number, rm: Pos[]) {
        this.DoubleRotateCollapse(delayTime, rm);
    }
    /**
     * 双中间环坍塌：
     * - 内环逆时针
     ...
     */
    private DoubleRotateCollapse(delayTime: number, removePos: Pos[]) {
        ...
        const hitInner = removePos.some(p => innerLoop.some(q => q.x === p.x && q.y === p.y));
