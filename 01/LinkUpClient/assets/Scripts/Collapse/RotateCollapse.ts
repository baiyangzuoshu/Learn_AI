...
export class RotateCollapse implements CollapseStrategy {
    collapse(delayTime: number, rm: Pos[]) {
        this.RotateCollapse(delayTime, rm);
    }

    RotateCollapse(delayTime: number, removePos: Pos[]) {
        ...
        if (removePos.some(p => CollapseUtil.isOnBorder(p, w, h))) {
    ...
    private rotateBorder(board: number[][]): MoveInfo[] {
        ...
        // 第一行
        for (let x = 0; x < w; x++) borderCoords.push({ x, y: 0 });
        // 右侧
        for (let y = 1; y < h; y++) borderCoords.push({ x: w - 1, y });
        // 底行（倒序）
        for (let x = w - 2; x >= 0; x--) borderCoords.push({ x, y: h - 1 });
        // 左侧（倒序，但不重复四角）
        for (let y = h - 2; y > 0; y--) borderCoords.push({ x: 0, y });
