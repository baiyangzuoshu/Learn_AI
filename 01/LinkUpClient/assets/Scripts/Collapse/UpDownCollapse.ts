...
export class UpDownCollapse implements CollapseStrategy {
    collapse(delayTime: number, rm: Pos[]) {
        this.UpDownCollapse(delayTime, rm);
    }

    UpDownCollapse(delayTime: number, removePos: Pos[]) {
        ...
        if (up.length > 0) {
            ...
            moveY = TitleSize;
            const list = CollapseUtil.collapseUp(board, up, height);
        ...
        if (down.length > 0) {
            ...
            util.Log("down", down);
            moveY = -TitleSize;
            const list = CollapseUtil.collapseDown(board, down, height);
            util.Log("list", list);
            //
            CollapseUtil.doAcion({ delayTime, list, moveX, moveY });
        }
