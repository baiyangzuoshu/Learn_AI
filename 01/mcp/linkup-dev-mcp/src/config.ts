...
export function loadConfig(): ConfigResult {
  ...
  try {
    ...
    return { error: createError(ErrorCode.CONFIG_INVALID, `Invalid JSON in config: ${(e as Error).message}`) };
  }
    ...
    if (!existsSync(dir)) {
      return {
        error: createError(
          ...
          'Not a valid LinkUpClient project (missing required directories)'
        ),
      };
    }
  }
  for (const file of requiredFiles) {
    if (!existsSync(file)) {
      return {
        error: createError(
          ErrorCode.PROJECT_NOT_FOUND,
          'Not a valid LinkUpClient project (missing project.json)'
        ),
      };
    }
  }
