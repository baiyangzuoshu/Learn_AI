...
import type { CDPClient, CDPTarget } from 'chrome-remote-interface';
...
interface ConsoleEntry {
  ...
  text: string;
  timestamp: number;
  stackTrace?: Array<{ functionName: string; url: string; lineNumber: number }>;
}

export class CDPManager {
  private config: CDPManagerConfig;
  private client: CDPClient | null = null;
  private target: CDPTarget | null = null;
  private connectedAt: string | null = null;
  private lastSeenAt: string | null = null;
  private consoleBuffer: ConsoleEntry[] = [];
  private consoleHandler: ((params: any) => void) | null = null;
  private _status: RuntimeStatus = { connected: false };
  ...
  async connect(): Promise<RuntimeStatus> {
    ...
    try {
      ...
      const cocosTarget = targets.find((t: CDPTarget) =>
        t.type === 'page' && (
          t.url.includes('cocos') ||
      ...
      if (!cocosTarget) {
        ...
        const pageTarget = targets.find((t: CDPTarget) => t.type === 'page');
        if (!pageTarget) {
          this._status = {
            connected: false,
   ...
   */
  async evaluateSafe(expression: string): Promise<any> {
    if (!this.client) {
      throw new Error('Not connected to Chrome DevTools');
    }

    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('CDP evaluation timed out')), this.config.timeoutMs)
    );
  ...
  async captureScreenshot(format: string = 'png', quality?: number): Promise<string> {
    if (!this.client) {
      throw new Error('Not connected to Chrome DevTools');
    }
  ...
  getConsoleLogs(options?: { level?: string; limit?: number; since?: number }): ConsoleEntry[] {
  ...
  private setupConsoleCapture(): void {
    ...
    this.consoleHandler = (params: any) => {
 ...
 */
export function createCDPManager(overrides?: Partial<CDPManagerConfig>): CDPManager {
