...
import type { ProjectContext, ProjectIndex } from '../project-context.js';
 ...
 */
function pathKindFromFunction(funcName: string): 'lookup' | 'button' | 'delay-button' | 'mouse' {
  switch (funcName) {
...
export async function handleResolveUiContract(
  ctx: ProjectContext,
  input: ResolveUiContractInput
): Promise<ToolEnvelope<any>> {
  const start = Date.now();
  const index = ctx.getIndex();

  // Find the prefab
  const matchingPrefabs = index.prefabs.filter(
    (p) => basename(p.relPath, '.prefab') === input.uiName
  );
  ...
  if (matchingPrefabs.length > 1) {
    return createErrorResponse(
      ...
      `Multiple prefabs found for "${input.uiName}": ${matchingPrefabs.map(p => p.relPath).join(', ')}`
    );
  }
  ...
  let uiNameKey: string | undefined;
  let uiNameValue: string | undefined;
  let uiNameLine: number | undefined;
  ...
  let uiControllerNameKey: string | undefined;
  let uiControllerNameValue: string | undefined;
  ...
  let uiControllerHandler: string | undefined;
  let uiControllerLine: number | undefined;
  let controllerRelPath: string | undefined;
  let controllerClassName: string | undefined;
  ...
  // Determine status
  let status: 'complete' | 'incomplete' | 'ambiguous' | 'not_found' = 'not_found';
  if (matchingPrefabs.length === 1) {
  ...
  try {
    ...
    diagnostics = checkResult.diagnostics.filter((d: any) => {
      if (d.file && d.file.includes(input.uiName)) return true;
      if (d.subject && d.subject.includes(input.uiName)) return true;
      return false;
    });
  } catch {
    // Non-fatal
