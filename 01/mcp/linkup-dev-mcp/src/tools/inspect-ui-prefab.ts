...
): { tree: any; count: number; returnedCount: number } {
  if (startIdx >= nodes.length) return { tree: null, count: 0, returnedCount: 0 };

  const startNode = nodes[startIdx];
  const totalNodes = nodes.length;
  let returnedNodes = 0;

  function buildNode(idx: number, depth: number): any | null {
    if (idx >= nodes.length || depth > maxDepth) return null;
    if (returnedNodes >= maxTreeNodes) return null;
    const n = nodes[idx];
    ...
    while (nextIdx < nodes.length) {
      if (nodes[nextIdx].depth <= n.depth) break;
...
export async function handleInspectUiPrefab(
  ctx: ProjectContext,
  input: InspectUiPrefabInput
  ...
  const { tree, count, returnedCount } = buildTreeNode(
    allNodes,
    0,
    maxDepth,
    ctx.maxTreeNodes,
  ...
  let registrationStatus: 'global' | 'local' | 'unregistered' | 'ambiguous' = 'unregistered';
  ...
  if (registrations.length > 0 && registrations[0].controllerImport) {
    ...
    if (ctrl) controllerRelPath = ctrl.relPath;
  }
