...
export async function handleRuntimeSceneTree(
  cdpManager: CDPManager,
  input: RuntimeSceneTreeInput
  ...
  try {
    const expression = buildSceneTreeExpression(
      input.maxDepth ?? 8,
      input.maxNodes ?? 2000
    );
    ...
    if (!result || result.error) {
      return createErrorResponse(
        ...
        result?.error || 'Failed to read scene tree from preview'
      );
    }
