...
export function readValidationRules(ctx: ProjectContext): { mimeType: string; text: string } {
  const ruleIds = listRules();

  const ruleDescriptions: Record<string, { description: string; severity: string }> = {
    'prefab/json-valid': {
