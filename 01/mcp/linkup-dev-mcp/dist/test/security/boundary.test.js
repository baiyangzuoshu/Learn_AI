...
describe('Security', () => {
    ...
    let transport;
    before(async () => {
        client = new Client({ name: 'security-test', version: '0.0.1' }, { capabilities: {} });
        ...
        for (const tool of result.tools) {
            ...
            if (inputSchema && inputSchema.properties) {
                for (const field of forbiddenFields) {
                    assert.strictEqual(inputSchema.properties[field], undefined, `Tool "${tool.name}" should not have forbidden field "${field}"`);
                }
            }
        }
    });
