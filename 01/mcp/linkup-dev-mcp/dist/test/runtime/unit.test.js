...
import { createCDPManager } from '../../src/runtime/cdp-manager.js';
import { buildSceneTreeExpression, buildNodeDetailExpression, buildStatusExpression, } from '../../src/runtime/whitelist.js';
describe('CDPManager', () => {
...
describe('Whitelist - buildNodeDetailExpression', () => {
    ...
    it('should reject paths with special characters', () => {
        assert.throws(() => buildNodeDetailExpression('node<script>'), /disallowed characters/);
        assert.throws(() => buildNodeDetailExpression('node;alert(1)'), /disallowed characters/);
        assert.throws(() => buildNodeDetailExpression('node"test'), /disallowed characters/);
    });
    it('should reject paths longer than 512 characters', () => {
        const longPath = 'a'.repeat(513);
        assert.throws(() => buildNodeDetailExpression(longPath), /too long/);
    });
