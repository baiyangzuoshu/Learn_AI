...
describe('Integration: Minimal Fixture', () => {
    ...
    before(async () => {
        const result = await createProjectContext({
            ...
            buildProjectIndex: (root) => buildProjectIndex(root),
            parsePrefab: (c, p) => parsePrefab(c, p),
            getRootNodeName: (p) => getRootNodeName(p),
            walkNodes: (p) => walkNodes(p),
            getComponents: (p) => getComponents(p),
            nodeHasComponent: (p, path, type) => nodeHasComponent(p, path, type),
            extractNodePaths: (c, p) => extractNodeNodes(c, p),
            listRules: () => listRules(),
        });
        ...
        const realResult = await createProjectContext({
            ...
            getComponents: (p) => getComponents(p),
            nodeHasComponent: (p, path, type) => nodeHasComponent(p, path, type),
            extractNodePaths: (c, p) => extractNodePaths(c, p),
            listRules: () => listRules(),
        });
