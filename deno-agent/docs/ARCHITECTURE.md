...
Deno Agent 是一个本地 AI 编程 Agent 桌面应用。它使用 Deno Desktop 和系统 WebView 承载界面，不引入
Electron、Node.js 运行时或 Rust 应用层。核心目标是把 `learn-claude-code` s01–s20 的 Agent Harness
能力整合进一个可运行、可扩展、可发布的桌面产品。

## 设计目标

- 本地优先：工作区文件、对话、Memory、任务图和定时任务都保存在本机。
- 单一 Agent Loop：所有工具、编排和定时任务共享 `AgentRuntime.run()`。
- 插件化 Harness：新增能力通过 Feature 注册工具和系统提示，不修改运行时核心。
- 权限可控：文件、Shell、后台任务和外部工具调用都经过权限模式与硬拒绝规则。
- 生产与教学隔离：`stages/` 只做学习示例，生产代码不能 import `stages/`。
...
desktop/renderer/           原生桌面 UI
  ↓ HTTP / NDJSON stream
desktop/main.ts             Deno Desktop 后端、HTTP API、静态资源服务
  ↓
src/harness/mod.ts          Harness 组合入口
  ↓
...
│   └── harness/             # Agent Harness 生产实现
│       ├── mod.ts           # Harness 组合根和对外入口
...
`src/harness/mod.ts` 是生产 Harness 的组合根：
...
- `agentLoop()`：面向 UI 的简化入口，负责把内部 Harness event 转成 UI 需要的事件。
