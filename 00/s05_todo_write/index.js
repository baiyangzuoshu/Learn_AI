    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          pattern: {
            type: "string",
            description: "Glob pattern such as **/*.js or s05_todo_write/*.md.",
          },
        },
        required: ["pattern"],
      },
...
function logHook(toolCall) {
  console.log(`\x1b[90m[HOOK] ${toolCall.name}\x1b[0m`);
  return null;
}
...
async function main() {
  console.log("s05: TodoWrite (Node.js + DeepSeek)");
  ...
  while (true) {
    const query = await terminal.question("\x1b[36ms05 >> \x1b[0m");
